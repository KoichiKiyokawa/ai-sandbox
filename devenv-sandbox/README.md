...
`devenv up` は devenv の process/task 依存関係で Postgres の起動を待ち、`pnpm db:push` と `pnpm db:seed` を実行してから Web サーバーを起動します。
...
スキーマは [src/db/schema.ts](/Users/koichi/workspace/ai-sandbox/devenv-sandbox/src/db/schema.ts) にあります。seed は [src/db/seed.ts](/Users/koichi/workspace/ai-sandbox/devenv-sandbox/src/db/seed.ts) にあります。マイグレーションは `pnpm db:generate` で生成し、`pnpm db:push` でローカル DB に反映します。初期データは `pnpm db:seed` で投入します。
